...
/** @brief Unique descriptor for the CLKNODE_GET_GATE() method */
extern struct kobjop_desc clknode_get_gate_desc;
/** @brief A function implementing the CLKNODE_GET_GATE() method */
typedef int clknode_get_gate_t(struct clknode *clk, bool *enabled);

static __inline int CLKNODE_GET_GATE(struct clknode *clk, bool *enabled)
{
	kobjop_t _m;
	int rc;
	KOBJOPLOOKUP(((kobj_t)clk)->ops,clknode_get_gate);
	rc = ((clknode_get_gate_t *) _m)(clk, enabled);
	return (rc);
}
