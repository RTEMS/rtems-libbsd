...
#ifdef __rtems__
#include "rtems-bsd-nuttcp-namespace.h"
#endif /* __rtems__ */

/*
 *	N U T T C P . C						v8.2.2
 *
 * Copyright(c) 2000 - 2018 Bill Fink.  All rights reserved.
 * Copyright(c) 2003 - 2018 Rob Scott.  All rights reserved.
 *
...
#if !defined(__CYGWIN__) && !defined(_WIN32) && !defined(__rtems__)
#define HAVE_SETPRIO
#endif
...
#include "rtems-bsd-nuttcp-nuttcp-data.h"
#endif
...
int af3 = 0;			/* used for third party explicitaf */
...
#ifdef __rtems__
RTEMS_LINKER_RWSET(bsd_prog_nuttcp, char);

/*
 * LibBSD on RTEMS does not support signals. Simulate something
 * to make nuttcp happy.
 *
 * In this implemnentation the alarm and real timers are separate.
 */
static rtems_id timer_worker_id;
static int timer_worker_running;
static volatile int timer_alarm_msecs;
static volatile struct itimerval timer_itimer;

int rtems_timer_setitimer(
  int which, const struct itimerval *value, struct itimerval *ovalue) {
  if (ovalue != NULL) {
    ovalue->it_interval.tv_sec = timer_itimer.it_interval.tv_sec;
    ovalue->it_interval.tv_usec = timer_itimer.it_interval.tv_usec;
    ovalue->it_value.tv_sec = timer_itimer.it_value.tv_sec;
    ovalue->it_value.tv_usec = timer_itimer.it_value.tv_usec;
  }
  if (value != NULL) {
    timer_itimer.it_interval.tv_sec = value->it_interval.tv_sec;
    timer_itimer.it_interval.tv_usec = value->it_interval.tv_usec;
    timer_itimer.it_value.tv_sec = value->it_value.tv_sec;
    timer_itimer.it_value.tv_usec = value->it_value.tv_usec;
  }
  return 0;
}
...
unsigned int rtems_timer_alarm(unsigned int seconds) {
  timer_alarm_msecs = seconds * 1000;
}

#define setitimer(w, t, ot) rtems_timer_setitimer(w, t, ot)
#define signal(s, f) rtems_timer_signal(s, f)
#define alarm(a) rtems_timer_alarm(a)

static void rtems_timer_worker(rtems_task_argument arg) {

  while (timer_worker_running) {
    usleep(1000);
    if (timer_alarm_msecs > 0) {
      --timer_alarm_msecs;
      if (timer_alarm_msecs == 0) {
	sigalarm(SIGALRM);
      }
    }
    if (timer_itimer.it_value.tv_sec != 0 ||
	timer_itimer.it_value.tv_usec != 0) {
      suseconds_t usec = timer_itimer.it_value.tv_usec;
      if (usec > 1000) {
        usec -= 1000;
      } else {
	if (timer_itimer.it_value.tv_sec > 0) {
	  --timer_itimer.it_value.tv_sec;
	}
        if (timer_itimer.it_value.tv_sec == 0) {
	  sigalarm(SIGALRM);
	  usec = 0;
          if (timer_itimer.it_interval.tv_sec != 0 ||
	      timer_itimer.it_interval.tv_usec != 0) {
	    timer_itimer.it_value.tv_sec = timer_itimer.it_interval.tv_sec;
	    usec = timer_itimer.it_interval.tv_usec;
	  }
        } else {
          usec = 1000000 - (1000 - usec);
        }
      }
      timer_itimer.it_value.tv_usec = usec;
    }
  }
...
static int rtems_timer_worker_start(void) {
  rtems_status_code sc;
  rtems_task_priority priority;
  rtems_name name;

  timer_worker_running = 0;
  timer_worker_id = 0;

  sc = rtems_task_set_priority(RTEMS_SELF, RTEMS_CURRENT_PRIORITY, &priority);
  if (sc != RTEMS_SUCCESSFUL) {
    printf("error: cannot get current priority: %s\n", rtems_status_text(sc));
    return 1;
  }

  name = rtems_build_name('T', 'T', 'C', 't');
  sc = rtems_task_create(name, priority - 1, 4 * 1024,
			 RTEMS_PREEMPT | RTEMS_TIMESLICE | RTEMS_NO_ASR,
                         RTEMS_FLOATING_POINT | RTEMS_LOCAL,
                         &timer_worker_id);
  if (sc != RTEMS_SUCCESSFUL) {
    printf("error: cannot create timer worker thread: %s\n", rtems_status_text(sc));
    timer_worker_id = 0;
    return 1;
  }
...
static void rtems_timer_worker_stop(void) {
  timer_worker_running = 0;
  while (timer_worker_id != 0) {
    usleep(10000);
  }
}
...
/*
 * Not in the BSD namespace
 */
#undef rtems_shell_main_nuttcp
int
rtems_shell_main_nuttcp(int argc, char *argv[])
{
  int exit_code;
  void *data_begin;
  size_t data_size;
  ...
  data_begin = RTEMS_LINKER_SET_BEGIN(bsd_prog_nuttcp);
  data_size = RTEMS_LINKER_SET_SIZE(bsd_prog_nuttcp);

  rtems_bsd_program_lock();
  stdin_copy = stdin;
  exit_code = rtems_timer_worker_start();
  if (exit_code == 0) {
    exit_code =
		...
		if (clientserver && (stream_idx == 1)) {
			...
			if (client) {
...
#else /* __rtems__ */
				/*
				 * RTEMS fd allocator means the dup() if may not be 0. It does
				 * not follow the standard.
				 * RTEMS has a number of references to fd 0 so a close may not close
				 * Hack around this
				 */
				stdin = fdopen(fd[0], "r");
#endif /* __rtems__ */
				...
				if ((irvers >= 80201) && host3) {
					...
					if (host3 && explicitaf) {
						fprintf(stdout, "nuttcp%s%s: third party \"-4/-6\" not supported by server version %d.%d.%d, need >= 8.2.1\n",
							trans?"-t":"-r",
							ident, rvers_major,
							rvers_minor,
							rvers_delta);
			...
			if (trans) {
				...
				if (udp) {
					...
					if (af == AF_INET) {
						...
						if (client && (strcmp(host, tmphost) != 0))
							fprintf(stdout, " ip=%s", tmphost);
					}
#ifdef AF_INET6
					else if (af == AF_INET6) {
				...
				if (srcport)
				    fprintf(stdout, " srcport=%d", srcport);
				fprintf(stdout, " mode=%s", udp ? "udp":"tcp");
				if (udp) {
					if (af == AF_INET) {
						fprintf(stdout, " af=inet");
					}
